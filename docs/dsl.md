...
## 文書全体の表現(`src/dsl/dslDocument.ts`)

ここまでの構文は、DSLパネルでの「選択を書き出し→編集→適用」というフラットな
編集用途のものです。これとは別に、CAD文書全体(要素・パレット・表示ロール/
プロファイル・印刷レイアウト・評価リミット)を無損失にテキスト化する文書
グラマーがあります。`serializeDocumentToDsl` / `parseDslDocument` が入口です。
現時点では保存形式はまだJSONで、この文書グラマーはストアには未接続です
(将来のPhaseで `.nui` 保存形式・エディタに接続される予定)。

### バージョン指令

文書は先頭の意味のある行(コメント・空行を除く)が必ず次の形式である必要が
あります。
...
未対応のメジャーバージョン、欠落、複数回の指定はエラーになります。

### ブロック構文

グループ・if/for・印刷レイアウトはブレース `{ }` のブロックとして書けます。
インデントは見た目のみで意味を持ちません。1文=1行を保つため、開き行・
メンバー行・閉じ行はそれぞれ独立した行です(`group A {}` のような同一行
での即時クローズはエラー)。
...
繰り返しは `for 名前 変数名 start=... count=... step=... { ... }` です。
...
キーワードの直後が `=` ・属性(`k=v`)・`{` のいずれかの場合、その文は無名
になります。無名要素は名前で参照できません。
...
```text
color pattern-black "#31322f" name="基本線" default
color cut-red "#b42318" name="裁断線"
```

`default` を付けた色が `defaultColorId` になります。

### 印刷レイアウト

```text
printLayout A4 output=pdf view=印刷 paper=a4 orientation=portrait
  columns=2 rows=2 overlap=10 scale=1 canvas=(410, 584) {
  layoutVar margin = 15
...
`place` はグループを配置し、`layoutVar` はそのレイアウトだけで使えるローカル
変数を定義します(`place` の式から `@名前` で参照可能)。`activePrintLayout`
は先頭のレイアウトが有効な場合は省略されます。無名のレイアウトが参照された
場合は、シリアライズ時に限り決定論的な名前(`レイアウト1` など)が自動で
付与されます。
...
`@stop` は、それより前に出現した要素文の数を評価対象として区切るマーカー
行です。文書に1つまでで、省略すると全要素が評価されます。ブロックの中に
書くこともできます。

### 過渡的な制約

要素の配列上の親子関係が非連続(グループの子が配列上で連続していない)な
場合、ブレースでは表現できないため、その要素だけ従来どおり `parent=` /
`branch=` 属性付きのトップレベル文として出力されます。これは将来のPhaseで
テキストが正準になれば構造的に発生しなくなる、過渡期のみの安全策です。
