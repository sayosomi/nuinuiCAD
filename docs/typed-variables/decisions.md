...
- typed numberの`const`/`let`初期化式・number型`set`右辺でも`@Element.property`を使える。コンパイル時に要素名をstable element ID・canonical property path・対象element source orderへ解決し、Rustを含むruntimeは名前を再解決しない。lazy/on-demand評価でも、initializerは宣言source order、set RHSはversion source orderを基準に後方参照を拒否する。未知要素・未知/非numeric propertyはcompile diagnostic、後方・disabled・invalid・geometry評価失敗はruntime diagnosticとする。
