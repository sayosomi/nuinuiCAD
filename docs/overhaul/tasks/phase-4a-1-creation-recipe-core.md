...
* promptは `parameterDefinitions` のラベルを既定とし、専用レシピは文言だけを
  上書きできる(keyやkindを変えない)。promptは空文字にしない。
...
* **parameterDefinitions整合テスト**: 全登録レシピの全ステップについて、
  ...
  あり、promptが空でないこと。乖離時にどのレシピのどのkeyかが分かるメッセージで落とすこと。
...
* 4a-2は本タスクの基盤の上に残り全型のレシピを載せる(基盤設計の変更
  ...
  ゴールデン追加だけでよい。
  `angleLengthLine` のフォールバックにある重複ステップと原点プレビューの
  問題は、専用レシピ化で扱う(4a-1では修正しない)。
