...
fn push_polyline(
    ...
    for (index, segment) in segments.iter().enumerate() {
        ...
        if index == 0 {
            let (x, y) = page_point(*start, origin);
...
pub fn encode_pdf(payload: &ResolvedPrintOutputPayload) -> Result<Vec<u8>, String> {
...
mod tests {
    ...
    fn builds_pages_in_payload_order_and_validates_payload() {
        let pdf = encode_pdf(&payload()).expect("pdf should build");
    ...
    #[test]
    fn emits_one_closed_pdf_path_for_a_polyline() {
    ...
    fn emits_exact_text_local_layout_for_multiline_rotated_mirrored_text() {
        ...
        let pdf = encode_pdf(&text_payload).expect("text PDF should build");
        let text = String::from_utf8_lossy(&pdf);
    ...
    fn full_pdf_build_propagates_unsupported_text_error() {
        ...
        let error = encode_pdf(&unsupported_payload)
    ...
    fn consumes_resolved_guide_center_and_keeps_label_off_the_guide_line() {
        ...
        let pdf = encode_pdf(&resolved).expect("resolved guide center should build");
        let text = String::from_utf8_lossy(&pdf);
