...
fn polyline_path_data(
    segments: &[OutputPathSegment],
    payload: &ResolvedSvgOutputPayload,
) -> Option<String> {
    let OutputPathSegment::Line { start, .. } = segments.first()? else {
        return None;
    };
    let (start_x, start_y) = svg_point(*start, payload);
    let mut data = format!("M {start_x} {start_y}");
    for segment in segments {
        let OutputPathSegment::Line { end, .. } = segment else {
            return None;
        };
        let (x, y) = svg_point(*end, payload);
        ...
        | OutputDrawable::JoinedPath {
            ...
            if let Some(fill) = fill {
                if let Some(data) = closed_segmented_path_data(segments, payload) {
                    push_path_data(svg, &data, stroke, Some(fill));
                }
            } else {
                for segment in segments {
...
pub fn encode_svg(payload: &ResolvedSvgOutputPayload) -> Result<String, String> {
...
mod tests {
    use super::*;
    use crate::output::payload::{text_bounds_relative, OutputBounds, OutputStroke};
    ...
    fn validates_payload_and_converts_y_axis_at_svg_boundary() {
        let svg = encode_svg(&payload()).expect("svg should build");
    ...
    fn rejects_invalid_payload() {
        ...
        assert!(encode_svg(&invalid).is_err());
    }

    #[test]
    fn emits_closed_polyline_paths() {
    ...
    fn emits_a_segmented_fill_as_one_closed_path() {
        ...
        assert!(
            svg.contains(r##"d="M 0 80 L 10 80 L 10 70 Z" fill="#123456" fill-opacity="0.25""##)
        );
    }
    ...
    fn emits_exact_text_local_layout_for_multiline_rotated_mirrored_text() {
        ...
        let svg = encode_svg(&text_payload).expect("text SVG should build");
        assert!(svg.contains(
