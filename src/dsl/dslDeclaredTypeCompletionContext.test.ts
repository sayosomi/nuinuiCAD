...
describe("declaredTypeCompletionContextAt", () => {
  it.each([
    ...
    expect(declaredTypeCompletionContextAt(source, pos)).toEqual({ from: expectedFrom, to: pos });
    expect(dslCompletionContextAt(source, pos)).toEqual({ kind: "declaredType", from: expectedFrom, to: pos });
  });

  it("uses the grammar's declared type names as the completion catalog", () => {
    expect(dslTypedDeclarationTypeNames).toEqual(["number", "string", "boolean", "choice"]);
  });
