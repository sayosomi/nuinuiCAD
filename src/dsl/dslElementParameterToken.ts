...
/**
 * Finds the `ElementName.query` (or nui 3 `@ElementName.query`) token ending
 ...
 * function with `elementToken === "@AB"` and silently produced zero
 * candidates (see expressionReferenceToken.ts and the Task 51 migration
 * plan).
 *
 ...
 * replace only the member token on pick and leave the element reference and
