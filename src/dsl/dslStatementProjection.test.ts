...
describe("statement source projection", () => {
  it("projects second-line values to their physical positions", () => {
    const source = ["point A = coordinate(", "  x: 0", "  y: 0", "  color: red", "  locked: true", ")"].join("\n");
    ...
    if (values.ok) expect(values.value.map((span) => source.slice(span.from, span.to))).toEqual(expect.arrayContaining(["red", "true"]));
  });
