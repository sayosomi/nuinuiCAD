...
describe("queryDslTypoSuggestions bisection", () => {
  it("reuses canonical keyword, type, construction, and construction-argument candidates", () => {
    ...
    const construction = ["nui 4", "point A = coordinate(x: 0, y: 0)", "point B = coordinate(x: 10, y: 0)", "line L = segmnt(start: @A, end: @B)"].join("\n");
    expect(labels(construction, "unknown-construction")).toContain("segment");
    const argument = ["nui 4", "point A = coordinate(x: 0, y: 0)", "point B = coordinate(x: 10, y: 0)", "line L = segment(strat: @A, end: @B)"].join("\n");
    expect(labels(argument, "unknown-construction-argument")).toContain("start");
    const mutation = ["nui 4", "point A = coordinate(x: 0, y: 0)", "point B = coordinate(x: 10, y: 0)", "line L = segment(start: @A, end: @B)", "move(targets: [@L], form: @A, to: @B)"].join("\n");
  ...
  it("reuses builtin callable and named-argument candidates", () => {
    ...
    expect(callable?.candidates).toEqual(expect.arrayContaining([expect.objectContaining({ kind: "builtin", label: "round", distance: 1 })]));
    const namedArgument = queryFor("nui 4\nconst value: number = spreadAngle(lenght: 10, spread: 20)", "unknown-function-argument").result;
    expect(namedArgument?.targetKind).toBe("builtinArgument");
    expect(namedArgument?.candidates).toEqual(expect.arrayContaining([expect.objectContaining({ kind: "argumentName", label: "length", distance: 1 })]));
  });

  it("uses the completion-owned edit range inside an exact @binding diagnostic span", () => {
    const source = ["nui 4", "const seamAllowance: number = 10", "const result: number = @seamAlowance"].join("\n");
    ...
    expect(result?.candidates).toEqual(expect.arrayContaining([expect.objectContaining({ kind: "binding", label: "seamAllowance", distance: 1 })]));
