...
const bezierSourceSegments = (curve: ComputedBezierCurve): SourceSegment[] =>
  curve.segments.flatMap((segment) => {
    const points = Array.from({ length: CURVE_OFFSET_STEPS + 1 }, (_, index) =>
      cubicPointAt(segment, index / CURVE_OFFSET_STEPS)
    );
    return points.slice(0, -1).flatMap((start, index) => {
      const end = points[index + 1];
      return lineLength(start, end) <= EPSILON ? [] : [{ kind: "line" as const, start, end }];
    });
  });
