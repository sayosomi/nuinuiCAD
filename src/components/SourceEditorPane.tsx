...
import { fileNameFromPath } from "@nuinuicad/nui-language/document";
...
export const SourceEditorPane = forwardRef<SourceEditorHandle, SourceEditorPaneProps>(function SourceEditorPane(
  ...
  useImperativeHandle(ref, () => ({
    ...
    currentSourceCursor: () => controllerRef.current?.currentSourceCursor?.() ?? null,
    ...
    currentCursorModuleSemanticTarget: () => controllerRef.current?.currentCursorModuleSemanticTarget?.() ?? null,
    jumpToModuleSemanticTarget: (target) => controllerRef.current?.jumpToModuleSemanticTarget(target) ?? false,
    goToSourceDefinitionAtCursor: () => controllerRef.current?.goToSourceDefinitionAtCursor?.() ?? false,
