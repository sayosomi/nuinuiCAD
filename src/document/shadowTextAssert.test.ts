...
describe("assertShadowEquivalent", () => {
  it("正準シリアライズが一致すればtrueを返しconsole.errorを呼ばない", () => {
    ...
    expect(assertShadowEquivalent(afterDoc, afterDoc, 1)).toBe(true);
  ...
  it("シリアライズが食い違えばfalseを返しconsole.errorで警告する", () => {
    const spy = vi.spyOn(console, "error").mockImplementation(() => {});
    const afterDoc = compileOrThrow(["nui 1", "point A = coordinate(x: 0, y: 0)"].join("\n"));
    const shadowDoc = compileOrThrow(["nui 1", "point A = coordinate(x: 99, y: 99)"].join("\n"));
    expect(assertShadowEquivalent(afterDoc, shadowDoc, 1)).toBe(false);
  ...
  it("shadowDocumentがnullならfalseを返しconsole.errorで警告する", () => {
    const spy = vi.spyOn(console, "error").mockImplementation(() => {});
    const afterDoc = compileOrThrow(["nui 1", "point A = coordinate(x: 0, y: 0)"].join("\n"));
    expect(assertShadowEquivalent(afterDoc, null, 1)).toBe(false);
    expect(spy).toHaveBeenCalledOnce();
  });
});

describe("assertReconcileSane", () => {
  const baseSource = ["nui 1", "point A = coordinate(x: 0, y: 0)", "point B = coordinate(x: 1, y: 1)", "point C = coordinate(x: 2, y: 2)"].join("\n");
  ...
  it("リネームのみ(移動なし)はID継承必須: 正常な場合は警告しない", () => {
    ...
    const nextShadowText = serializeDocumentToDsl(afterDoc, 1);
  ...
  it("prevCompiled.documentがnullなら比較対象がないので何もしない", () => {
    ...
    const afterDoc = compileOrThrow(["nui 1", "point A = coordinate(x: 0, y: 0)"].join("\n"));
    assertReconcileSane(brokenCompiled, serializeDocumentToDsl(afterDoc, 1), afterDoc);
    expect(spy).not.toHaveBeenCalled();
  });
