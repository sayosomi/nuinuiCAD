...
describe("VS Code current runtime evaluation boundary", () => {
  it("does not derive a source-order evaluation limit", async () => {
    ...
    expect(snapshot.compiled.document.evaluationLimitIndex).toBeUndefined();
    expect(snapshot.evaluation.evaluatedElementIds?.has(beforeStop.id)).toBe(true);
    expect(snapshot.evaluation.evaluatedElementIds?.has(afterStop.id)).toBe(true);
    expect(snapshot.evaluation.computedGeometry.has(afterStop.id)).toBe(true);
  });
});
