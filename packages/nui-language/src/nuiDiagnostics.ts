...
export type NuiDiagnosticRange = {
  start: NuiDiagnosticPosition;
  end: NuiDiagnosticPosition;
};

export type NuiDiagnosticRelatedInformation = {
  message: string;
  range: NuiDiagnosticRange;
  presentation?: DslDiagnosticPresentation;
};

/** Host-neutral diagnostic projection. Ranges use normalized-LF UTF-16 offsets. */
export type NuiDiagnostic = {
  ...
  range: NuiDiagnosticRange;
  relatedInformation?: readonly NuiDiagnosticRelatedInformation[];
  code?: string;
  source: "nuinuiCAD";
};
...
const positionAt = (index: LineIndex, offset: number): NuiDiagnosticPosition | null => {
  ...
  while (low <= high) {
    ...
    if (index.starts[middle]! > offset) high = middle - 1;
    else low = middle + 1;
  }
...
export const toNuiDiagnostic = (
  ...
  let range: NuiDiagnosticRange | null = null;
  ...
  const relatedInformation = (diagnostic.relatedInformation ?? [])
    ...
    .filter((related): related is NuiDiagnosticRelatedInformation => related !== null);
...
export const nuiDiagnosticsFor = (
  sourceText: string,
  diagnostics: readonly DslDiagnostic[],
  bindingIssueDiagnostics: readonly DslDiagnostic[]
): NuiDiagnostic[] => [...diagnostics, ...bindingIssueDiagnostics]
