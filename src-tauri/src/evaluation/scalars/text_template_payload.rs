...
/// Decodes and validates the whole `text_templates` array. `scalar_program_present`
...
/// because a typed declaration exists, which implies a `scalar_program` -
/// if the caller has no `scalar_program` at all but a typed hole is present
...
pub(crate) fn validate_text_templates_payload(
    ...
    {
        return Err(issue(
            Code::TypedHoleRequiresScalarProgram,
            "a typed text template hole requires a scalar program to be present",
        ));
    }
    Ok(decoded)
}
