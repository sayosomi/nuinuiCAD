...
fn decode_text_property_binding(
    ...
    if !valid_binding_ids.contains(binding_id.as_str()) {
        return Err(issue(
            ...
            format!(
                "text property binding bindingId \"{binding_id}\" does not exist in the scalar program"
            ),
        ));
    }
...
/// skipped, and an empty/absent `valid_binding_ids` (no `scalar_program`)
/// fails every entry closed rather than falling back to a literal value -
/// a bare `text.text` binding is always typed content, so it can never be
/// valid without a scalar program.
