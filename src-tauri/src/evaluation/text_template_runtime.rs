...
use super::scalars::{
    evaluate_text_template_segments, LegacyHoleEvaluator, ScalarBindingResolver, ScalarEvaluation,
    ScalarEvaluationEnvironment, TextTemplateHoleOrigin, ValidatedTextTemplate,
};
...
struct TextTemplateRuntimeContext<'a> {
    /// `None` whenever the document has no `scalar_program` at all - only
    ...
    resolver: Option<&'a ScalarBindingResolver<'a>>,
    element: &'a Value,
...
impl ScalarEvaluationEnvironment for TextTemplateRuntimeContext<'_> {
    fn lookup_binding(&self, binding_id: &str) -> ScalarEvaluation {
        let resolver = self.resolver.expect(
            "scalar_binding_resolver must exist when a text template has a typed hole - text_template_payload.rs's decode already rejects a typed hole with no scalar_program",
        );
        resolver.resolve(binding_id, self.state)
    }
}
...
pub(crate) fn resolve_text_template(
    ...
    resolver: Option<&ScalarBindingResolver>,
    state: &mut EvaluationState,
) -> Option<String> {
